...
func CreatePipeline(codecName string) *Pipeline {
	...
	switch codecName {
	case "VP8":
		pipelineStr += ", payload=96, encoding-name=VP8-DRAFT-IETF-01 ! rtpvp8depay ! decodebin ! autovideosink"
	case "opus":
		pipelineStr += ", payload=111, encoding-name=OPUS ! rtpopusdepay ! opusdec ! audioconvert ! autoaudiosink"
	case "vp9":
